...
import {View, Text, StyleSheet, Image, TextInput, ScrollView, Pressable, Modal, FlatList, TouchableWithoutFeedback, ActivityIndicator, TouchableOpacity, Alert, KeyboardAvoidingView, Keyboard, LogBox} from 'react-native'
import { AntDesign, Entypo , SimpleLineIcons, Feather, FontAwesome, MaterialIcons } from '@expo/vector-icons'
import DatePicker from 'react-native-date-picker'
import { fetchDbHostFailure, fetchDbHostRequest, fetchDbHostSuccess } from '../../Redux/dbHost/dbHostActions';
...
const HostProfile = ()=>{
  const { hostAuth } = useAuthContext() // host auth states and info from context api 
    const dbHost = useSelector( state => state.host.dbHost)
    const error = useSelector( state => state.host.error)
    const imageData = useSelector( state => state.hostImage.hostImageData)
    const dispatch = useDispatch()
    const [Loading, setLoading] = useState(false)
    const nameParts = dbHost? dbHost.name.split(' ') : [null, null]; // Split the user's name into first and last name, or set to [null, null] if dbUser is not defined
    ...
    const [firstName, setFirstName] = useState(dbHost? theFirstName : '') // Initialize state with the first name if dbUser exists, otherwise use an empty string
    const [lastName, setLastName] = useState(dbHost? theLastName : '') // Initialize state with the last name if dbUser exists, otherwise use an empty string
    const [dob, setDOB] = useState(dbHost? dbHost.dob : 'Select date of birth') // stores user date of birth
    const [Email, setEmail] = useState(dbHost? dbHost.email : '') // stores user email
    const [Country, setCountry] = useState(dbHost? dbHost.country : 'Select country') // stores user country
    const [State, setState] = useState(dbHost? dbHost.state : 'Select state') // stores user state
    ...
    const [selectedCountryNameCode, setSelectedCountryNameCode] = useState(dbHost? dbHost.countryCode : null) // stores country name code
    ...
    const [telephone, setTelephone] = useState(dbHost? dbHost.telephone : '') // stores user telephone number
    const [whatsapp, setWhatsapp] = useState(dbHost? dbHost.whatsapp : '' ) // stores user whatsapp number
    const [language, setLanguage] = useState(dbHost? dbHost.language : '')
    const [secLanguage, setSecLanguage] = useState(dbHost? dbHost.secondLanguage : '')
    ...
    const [ethnicity, setEthnicity] = useState(dbHost? dbHost.ethnicity : '') // stores user ethnicity
    ...
    const [callingCode, setCallingCode] = useState(dbHost? dbHost.callingCode : ''); // stores the country calling code
    const [disableProfileText, setDisableProfileText] = useState(dbHost? true : false) // Controls whether the form fields are editable or not (true = not editable, false = editable)
   ...
   const validateLanguageInput = ()=>{
    if(language.length == 0){
        // Set the language input error state to true if the name is invalid
       setLanguageInputError(true)
     }
   }
...
const checkAllFormFields = () => {
 ...
 // Handles all form data validation checks
   ...
   */
    checkAllFormFields()
   }, [firstName, lastName, Email, dob, Country, State, emailInputError, dateInputError, nameInputError, image, ethnicity, telephone, whatsapp, callingCode, language, selectedCountryNameCode])
...
const uploadProfilePicture = async (img, userId, UserData, mode) => {
    try {
      ...
      const result = await uploadData({
        key: `hostprofilePicture/${userId}/hostPhoto.jpeg`,
        data: theBlob,
        options: {
      ...
      if(mode == 'create'){
        dispatch(fetchDbHostFailure('create'))
        setSaveChanges(true)
        setDisableProfileText(false)
      }else{
        dispatch(fetchDbHostFailure('update'))
        setSaveChanges(true)
        setDisableProfileText(false)
      }
    }
   ...
   // Updates the user data in the backend database via DataStore and handles post-update actions based on the mode.
    const updateUserData = async (UserData, mode)=>{
        try{
            ...
            const theUserArr = await DataStore.query(Host, (u)=> u.sub.eq(UserData.sub))
            console.log('Final Update Host : ', theUserArr[0])
            ...
            const updatedUserData = await DataStore.save(
                Host.copyOf( theUserArr[0], (updated)=>{
                    updated.key = `hostprofilePicture/${ theUserArr[0].id}/hostPhoto.jpeg`
                })
            )
            ...
            if(mode == 'create'){
                ...
                dispatch(fetchDbHostSuccess(updatedUserData))
                setLoading(false)
...
// Handles user email update
    ...
    const setTelephoneChange = (tel)=>{
      ...
      const setSecLanguageChange = (val)=>{
          setSecLanguage(val)
      }
 ...
 */
    ...
    const pickImage = async (mode) => {
      ...
      /**
             ...
             * 
             * If a database user (`dbHost`) is present, the function displays an alert informing that the image is a required field.
             * If no `dbHost` is present, the function clears the selected image by setting the image state to `null`.
 */
      const deleteImage = ()=>{
      ...
      /**
             ...
             * 
             * - If a `dbHost` exists, it updates the existing user data by calling `updateUser()`.
             * - If no `dbHost` is present, it creates new user data by calling `createUser()`.
             * 
    ...
    */
      const createUser = async ()=>{
       ...
       try { 
        ...
        const UserData = await DataStore.save(new Host({
            ...
            sub: hostAuth, // User authentication ID
            language: language.trim(),
            secondLanguage: secLanguage?.trim()
        }))
 ...
 */
      const updateUser = async ()=>{
       try { 
        ...
        const theUserArr = await DataStore.query(Host, (u)=> u.sub.eq(hostAuth))
        ...
        const updateddUserData = await DataStore.save( 
            Host.copyOf( theUser, (updated)=>{
                updated.name =  `${firstName} ${lastName}`.trim(),
                updated.email  = Email.trim(),
                updated.telephone  = telephone.trim(),
                updated.whatsapp  = whatsapp.trim(),
                updated.callingCode  = callingCode,
                updated.countryCode = selectedCountryNameCode,
                updated.dob = dob,
                updated.country = Country,
                updated.state = State,
                updated.ethnicity = ethnicity.trim(),
                updated.sub = hostAuth
                updated.language = language.trim(),
                updated.secondLanguage = secLanguage?.trim()
        }))
        ...
        <KeyboardAvoidingView behavior='padding' style={{flex:1}}>
         <View style={styles.profileContainer}>
           ...
           <View style={styles.imageContainer}>
              ...
              <View style={{...styles.userImageCont, borderColor:disableProfileText? 'gray' : '#FF0000'}}>
             
                {image? <FastImage source={{uri : image, priority: FastImage.priority.high}} style={styles.image} resizeMode={FastImage.resizeMode.cover} /> : <Image source={placeholderProfileImage} style={styles.image}/>}
            </View>
            <Entypo name="camera" size={30} color={disableProfileText? "gray" : "#FF0000"} style={{right: 50, top: 175}} onPress={openBottomSheet}/>
         </View>
            <View style={{marginVertical: 30}}>
                ...
                <View style={{marginBottom: 15}}>
                    ...
                    <TextInput editable={disableProfileText? false : true} style={{...styles.txtInput, color: disableProfileText? '#4C4C4C' : 'black'}}  onBlur={checkEmailInput}  value={Email} onChangeText={setEmailChange} placeholder='email' autoCorrect={false}/>
              ...
              <View style={styles.nameContainer}>
                    <Text style={styles.label}>Language</Text>
                     <TextInput editable={disableProfileText? false : true} style={{...styles.txtInput, color: disableProfileText? '#4C4C4C' : 'black'}}  onBlur={validateLanguageInput}  value={language} onChangeText={setLanguageChange} placeholder="Enter Language" autoCapitalize='words' autoCorrect={false}/>
                    {languageInputError && <Text style={styles.errorText}>Language is Required!</Text>}
                </View>
               <View style={styles.nameContainer}>
                  <Text style={{...styles.label}}>Other lingo</Text>
               <TextInput editable={disableProfileText? false : true} style={{...styles.txtInput, color: disableProfileText? '#4C4C4C' : 'black'}} value={secLanguage} onChangeText={setSecLanguageChange} placeholder="Enter Language" autoCapitalize='words' autoCorrect={false}/>
              </View>
                </View>
         ...
         <View style={styles.profilePhotoOptionsContainer}>
           ...
           <Modal visible={error !== false? true : false} onRequestClose={()=>{dispatch(fetchDbHostFailure(false))}} presentationStyle='overFullScreen' transparent={true}>
           <StatusBar style="light" backgroundColor={'rgba(0,0,0,0.5)'}/>
                  <View style={{ flex: 1, ...styles.userProfileAlertContainer}}>
                        <View style={styles.userProfileSuccessAlert}>
                                ...
                                <Pressable onPress={()=>{dispatch(fetchDbHostFailure(false))}} style={{backgroundColor: 'black', ...styles.closeProfileAlert}}>
              ...
              <StatusBar style="light" backgroundColor={'rgba(0,0,0,0.5)'}/>
                  <View style={{ flex: 1, ...styles.userProfileAlertContainer}}>
                        <View style={{marginHorizontal: 30, ...styles.userProfileSuccessAlert}}>
                                <AntDesign name="exclamationcircleo" size={40} color="red" style={{marginBottom: 10}}/>
                                <Text style={{color:"red", ...styles.errorAlertLabel}}>Image is a required field</Text>
