...
import { storeHostImageData } from '../../Redux/hostImageData/hostImageActionsType';
...
const Profile = ()=>{
  ...
  const goToProfile = ()=>{
     navigation.navigate('ProfileView', {imageData})
  }
  ...
  const getJoinDate = ()=>{
    const localeDateString = dbHost.date; // Example localeDateString in MM/DD/YYYY format
...
const getProfilePictureUrl = async (imageUrl)=>{
    try{
      ...
      dispatch(storeHostImageData(`${URL}${encoded}`))
    }catch(e){
    ...
    return (
        ...
        <View style={{...styles.profileOverviewContain, padding: dbHost? 30 : 20}}>
           <View style={styles.userImageCont}>
                {dbHost? imageData? <FastImage source={{uri : imageData, priority: FastImage.priority.high}} style={styles.userImage} resizeMode={FastImage.resizeMode.cover}/> : <View style={{width: 180, height: 180, borderRadius: 180, backgroundColor:'white', justifyContent:'center', alignItems:'center'}}><Text style={{fontSize: 100, color:'#F52F57', fontWeight:'600'}}>{dbHost.name[0]}</Text></View>: <FastImage source={placeholderProfileImage} style={styles.userImage} resizeMode={FastImage.resizeMode.cover}/>}
            </View> 
           
            {!dbHost? <View style={styles.profileViewTextContainer}>
                  ...
                  <View style={styles.infoTextContainer}> 
                      ...
                      <Entypo name="edit" size={18} color="#4C4C4C" onPress={goToProfile}/>
                          ...
                          <Text  style={styles.userName}>{dbHost.name}</Text>
                          <Text style={styles.infoText}>{dbHost.email}</Text>
                            <View style={styles.userBioDataContainer}>
                                   <Entypo name="location-pin" size={24} color="#4C4C4C" />
                                   <Text style={styles.infoText}>{dbHost.state}, {dbHost.country}</Text>
                            </View>
        ...
        {!dbHost && <View style={styles.profilePurposeContainer}>
          <View>
          <Text style={{fontSize: 14}}>Create your profile today to unlock a more personalized and secure apartment listing experience.{ !showText && <Text style={{fontSize: 14, fontWeight: '700'}} onPress={()=>{setShowText(true)}}>Show more <FontAwesome6 name="chevron-down" size={14} color="black" /></Text>}</Text>
         { showText && <View>
            <Text style={styles.profileHighlightDesc}>By setting up a profile, you'll be able to : </Text>
            <View style={styles.profileHighlight}>
            <Ionicons name="radio-button-on-sharp" size={16} color="black" style={{marginRight: 5}} />
            <Text style={{fontSize: 14}}>Build trust with renters on the platform</Text>
            </View>

            <View style={styles.profileHighlight}>
            <Ionicons name="radio-button-on-sharp" size={16} color="black" style={{marginRight: 5}}/>
            <Text style={{fontSize: 14}}>Enable direct payment into your bank account</Text>
            </View>

            <View style={styles.profileHighlight}>
            <Ionicons name="radio-button-on-sharp" size={16} color="black" style={{marginRight: 5}}/>
            <Text style={{fontSize: 14}}>List apartments</Text>
            </View>

            <View style={styles.profileHighlight}>
            <Ionicons name="radio-button-on-sharp" size={16} color="black" style={{marginRight: 5}}/>
            <Text style={{fontSize: 14}}>Receive user reviews and comment about their rental experience with you</Text>
            </View>
            <Text style={{fontSize: 14}}>Create your profile now and enhance your apartment listing journey</Text>
          </View>}
          </View>
        </View>}
        ...
        <Pressable onPress={goToProfile} style={{...styles.goToProfileBtn, marginTop: dbHost? 40 : 0}}>
          <Text style={styles.goToProfileText}>{dbHost? 'View profile' : 'Create profile'}</Text>
        </Pressable>
        </ScrollView>
    </View>
