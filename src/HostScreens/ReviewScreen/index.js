    ...
    return(
       <View style={styles.container}>
          <StatusBar style='dark' backgroundColor='white'/>
              <FlatList data={allAgentRev}
              ListHeaderComponent={ReviewHeader(averageRevStar, showModalSort, setShowModalShow, setAllAgentRev, allAgentRev, AllAgentRev)} 
              renderItem={({item})=>{
                 return(
                  <Reviews item={item} mode={'vertical'} averageReviewStar={ averageRevStar } Agent_Reviews={allAgentRev}/>
           )
...
const ReviewHeader = (averageRevStar, showModalSort, setShowModalShow, setAgent_rev, agent_rev, Agent_Reviews)=>{
    ...
    <View>
         <View style={{ padding : 40, alignItems:'center', borderBottomWidth: 1, borderBottomColor:'lightgray'}}>
            <View style={styles.alignContainer}>
                    <Entypo name="star" size={40} color="black" style={{marginRight: 10}}/>
                    <Text style={styles.headerText}>{Agent_Reviews?.length>=2? averageRevStar?.toFixed(1) : 'New'} &#8226; {Agent_Reviews?.length == 0 || !Agent_Reviews? 0 : Agent_Reviews?.length} <Text style={{fontSize: 25}}>reviews</Text></Text>
            </View>
                      ...
                      <View style={styles.modalContainer}>
                        ...
                        <View style={{paddingVertical: 10}}>
                          <TouchableOpacity onPress={()=>{
                            ...
                            <Text style={{fontSize: radBtnValue == 'Recent'? 18 : 16, letterSpacing: 0.5, fontWeight: radBtnValue == 'Recent'? '500' : '400'}}>Most Recent</Text>
                            {radBtnValue == 'Recent'?  <Ionicons name="radio-button-on" size={25} color="black" /> : 
                            <Ionicons name="radio-button-off" size={25} color="gray" />}
                          </TouchableOpacity>
                          <TouchableOpacity onPress={()=>{ 
                              setRadBtnValue('High') 
                            }} 
                              style={styles.sortButtons}>
                            <Text style={{fontSize: radBtnValue == 'High'? 18 : 16, letterSpacing:0.5, fontWeight:radBtnValue == 'High'? '500' : '400'}}>Highest Rated</Text>
                            {radBtnValue == 'High'?  <Ionicons name="radio-button-on" size={25} color="black" /> : 
                            <Ionicons name="radio-button-off" size={25} color="gray" />}
                          </TouchableOpacity>
                          ...
                          <TouchableOpacity onPress={()=>{ 
                            ...
                            <Text style={{fontSize: radBtnValue == 'Low'? 18 : 16, letterSpacing:0.5, fontWeight:radBtnValue == 'Low'? '500' : '400'}}>Lowest Rated</Text>
                            {radBtnValue == 'Low'?  <Ionicons name="radio-button-on" size={25} color="black" /> : 
                            <Ionicons name="radio-button-off" size={25} color="gray"  style={{borderColor: 'red'}} />}
                          </TouchableOpacity>
                        </View>
